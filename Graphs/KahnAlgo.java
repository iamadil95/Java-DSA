...
public class KahnAlgo {
    ...
    static void createGraph(ArrayList<Edge> graph[]) { //Initializes each index of the adjacency list (graph) with an empty ArrayList
        for (int i = 0; i < graph.length; i++) {
            graph[i] = new ArrayList<>();
        }
//graph[2].add(new Edge(2, 3)) means 2 → 3
    ...
    public static void topSortKahn(ArrayList<Edge> graph[]) {
        ...
        while (!q.isEmpty()) {
            ...
            for (int i = 0; i < graph[curr].size(); i++) {
                ...
                indeg[e.dest]--; //neigbour indegree -1
                if (indeg[e.dest] == 0) {
                    q.add(e.dest);
                }
            }
